...
#the db engine we wish to communicate with
engine = create_engine('sqlite:///restaurantmenu.db')

# Bind the engine to the metadata of the Base class so that the
# declaratives can be accessed through a DBSession instance
#makes connection between class definitions and corresponding tables in db
Base.metadata.bind = engine

#link between code executions and the engine we created above
DBSession = sessionmaker(bind=engine)

# sql alchemy creates an interface called a session to allow us to act on the data in our db
# stages changes that will not be applied until 
# session.commit()
...
restaurant1 = Restaurant(name="Urban Burger") #instance of restaurant class
